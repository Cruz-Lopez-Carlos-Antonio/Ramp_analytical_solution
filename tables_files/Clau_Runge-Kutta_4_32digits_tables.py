...
if __name__ == "__main__":
    ...
    # The output is written in two tables, without the time column.
    # Each value is printed with 32 significant digits, matching mp.mp.dps = 32.
    with open("RK4_output_32digits.txt", "w", encoding="utf-8") as f:
        f.write("# n(t)\n")

        for t_req in times:
            idx = int(mp.nint((mp.mpf(t_req) - t0)/h))
            if 0 <= idx < len(T):
                f.write(mp.nstr(Nvals[idx], n=32, strip_zeros=False) + "\n")
        ...
        for t_req in times:
            idx = int(mp.nint((mp.mpf(t_req) - t0)/h))
            if 0 <= idx < len(T):
                f.write(mp.nstr(Cvals[idx], n=32, strip_zeros=False) + "\n")
