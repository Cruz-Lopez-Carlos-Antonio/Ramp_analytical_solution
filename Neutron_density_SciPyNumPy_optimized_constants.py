...
def Analytic_n(t, rho_0, beta, Lambda_1, gamma_1, lambda_1, q, A1, A2):
    # Eq. (49) 
    # A1 and A2 are integration constants determined once from the initial conditions.
    # They do not depend on time; only zeta, I_1, I_2, and I_5 depend on t.
    mu   = lambda_1*beta/gamma_1
...
if __name__ == "__main__":
    ...
    # A1 and A2 are determined only once from the initial conditions.
    A1, A2 = Constants_Ini_con(
        lambda_1, beta, Lambda_1, gamma_1, rho_s, n0, 0.0, source
    )

    times = range(0, 21)
    for k in times:
        val = Analytic_n(k, rho_s, beta, Lambda_1, gamma_1, lambda_1, source, A1, A2)
        print(val)
